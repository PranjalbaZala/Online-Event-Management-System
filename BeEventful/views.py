...
from .forms import ExtendedUserCreationForm, ProfileForm, EventForm, StaffForm , FeedbackForm, BookingForm
from .models import EventDetails, StaffDetails, FeedbackDetails,Booking
...
#For accessing event details a user must login first
...
#For accessing Event form / List admin must login first
    ...
    if request.method=="GET":
        if id==0:
           feedback_form = FeedbackForm()
            ...
            feedback_form = FeedbackForm(instance=feed)
        ...
        if id==0:
            ...
            feed=FeedbackDetails.objects.get(pk=id)
            feedback_form = FeedbackForm(request.POST,instance=feed)
...
def feedback_delete(request,id):
    feed=FeedbackDetails.objects.get(pk=id)
    feed.delete()
    return redirect('ApprovedFeedback_list')
